...
- `GET transfermarkt.es/ceapi/performance-game/{id}` — **JSON limpio** con **una fila por jugador-partido de toda su carrera** (232 de Forés), en todas las competiciones a la vez (La Liga, Segunda, Primera Federación, Copa, europeas). Espejo idéntico en `tmapi.transfermarkt.technology/player/{id}/performance-game`; usamos el de `transfermarkt.es` por ser el mismo host y UA que valores y traspasos. Cada fila trae: **estado de participación** (`played` / `in squad` / `not in squad` / `injured`), minutos, titular/suplente y minuto de cambio, goles y asistencias, tarjetas, y estadística de evento básica (pases, duelos, faltas). Marca la lesión del partido con `injuryId`/`absenceId`.
